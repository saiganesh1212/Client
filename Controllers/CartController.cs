    ...
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Cart()
        {
            ...
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    ...
                    using (var response1 = await client.GetAsync("http://52.147.219.188/api/cart/" + HttpContext.Session.GetString("id")))
                    {
        ...
        public async Task<IActionResult> AddToCart(int id)
        {
            ...
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    ...
                    using (var response = await client.GetAsync("http://52.147.217.69/api/menu/" + id.ToString()))
                    {
                    ...
                    client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    using (var response1 = await client.PostAsync("http://52.147.219.188/api/cart/addtocart/" + HttpContext.Session.GetString("id"), content))
                    {
                        if (!response1.IsSuccessStatusCode)
                        {
        ...
        public async Task<IActionResult> DeleteFromCart(int menuItemId)
        {
            ...
            else
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    using (var response1 = await client.DeleteAsync("http://52.147.219.188/api/cart/removeitem/" + HttpContext.Session.GetString("id") + "/" + menuItemId.ToString()))
                    {
                        if (!response1.IsSuccessStatusCode)
                        {
