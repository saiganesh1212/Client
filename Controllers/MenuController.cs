    ...
    public class MenuController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    ...
                    using (var response = await client.GetAsync("http://52.147.217.69/api/menu/allitems"))
                    {
        ...
        public async Task<IActionResult> EditMenuItem(int menuId)
        {
            ...
            else
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    using (var response1 = await client.GetAsync("http://52.147.217.69/api/menu/" + menuId))
                    {
        ...
        public async Task<IActionResult> EditMenuItem(int menuId, MenuItem menuItem)
        {
            ...
            new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("token"));
                    ...
                    using (var response1 = await client.PostAsync("http://52.147.217.69/api/menu/edititem/" + menuId, content))
                    {
                        if (!response1.IsSuccessStatusCode)
                        {
